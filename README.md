...
- [ ] 요구사항 분석: 보드의 제목과 현재 진행률(완료율)을 표시하며, 새로운 태스크를 추가할 수 있는 버튼을 제공합니다.
- [ ] UI 명세:
    - 배경색: 흰색
    - 패딩: 상하 17px, 좌우 24px
...
- [ ] 요구사항 분석: 태스크의 상태에 따라 3개의 컬럼(To-Do, In Progress, Done)으로 구분하여 카드를 배치합니다.
- [ ] UI 명세:
    - 컬럼 패딩: 17px
...
- [ ] 상태 분류: `KanbanStatus` 값에 따른 필터링 및 컬럼별 배치
- [ ] 완료율 계산: `(Done 상태 태스크 수 / 전체 태스크 수) * 100` (전체 0개일 시 0% 처리)
- [ ] 태스크 생성: 모달의 '생성' 버튼 클릭 시 새로운 `KanbanCard` 객체 생성 및 리스트 추가

### 4. UI 테스트

- [ ] 모달 연결: '새 태스크 생성' 버튼 클릭 시 Dialog 오픈 확인
- [ ] 폼 초기화: Dialog를 닫고 다시 열었을 때, 이전 입력값이 사라지고 기본값으로 복구되는지 확인
- [ ] 스낵바 알림: 카드 추가 성공 시 하단에 스낵바 생성 확인
- [ ] 스낵바 소멸: 노출 후 일정 시간이 경과하면 자동으로 화면에서 사라지는지 확인
